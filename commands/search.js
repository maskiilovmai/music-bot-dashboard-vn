...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!SearchString)
      return client.sendTime(
        message.channel,
        `**Sử dụng - **\`${GuildDB.prefix}search [nội dung cần tìm]\``
      );
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let Pages = songs.map((songz) => {
        let MappedSongs = songz.map(
          (s) =>
            `\**${s.index + 1}.**\ [${s.title}](${
              s.uri
            }) \n**Tác giả: **\`${s.author}\` **|** **Thời lượng: **\`${prettyMilliseconds(s.duration, {
              colonNotation: true,
            })}\``
        );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Bạn đã mất quá nhiều thời gian để trả lời. Chạy lại lệnh nếu bạn muốn phát nhạc theo tìm kiếm!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Vui lòng gửi chính xác số ID bài hát"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Không tìm thấy bài hát với ID đã gửi"
        );
      ...
      SongAddedEmbed.setDescription(`**Bài hát: **[${Song.title}](${Song.uri})`);
      SongAddedEmbed.addField("Tác giả", `\`${Song.author}\``, true);
      SongAddedEmbed.addField("Yêu cầu bởi", `${Song.requester}`, true)
