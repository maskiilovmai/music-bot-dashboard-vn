...
module.exports = {
  name: "move",
  description: "Di chuyển một bài hát đến một vị trí được chỉ định.",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "❌ | **Số bài nhạc không hợp lệ.**");
    }
    ...
    if (dest < 1 || dest > player.queue.length - 1) {
      return client.sendTime(
        message.channel,
        "❌ | **Điểm đến của bài hát không hợp lệ.**"
      );
    }
