...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return message.channel.send("Không có bài hát nào trong hàng đợi để xóa");
    let rm = new MessageEmbed()
      .setDescription(
        `✅ **|** Đã xóa bài nhạc **\`${Number(args[0])}\`** ra khỏi hàng chờ!`
      )
      .setColor("GREEN");
    if (isNaN(args[0]))
      rm.setDescription(
        `**Sử dụng - **${client.botconfig.prefix}\`remove [số thứ tự bài hát]\``
      );
