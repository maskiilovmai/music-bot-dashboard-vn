...
module.exports = {
  name: "queue",
  description: "Hiển thị tất cả bài hàng trong hàng chờ",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue === 0) {
      let QueueEmbed = new MessageEmbed()
        ...
        .setDescription(
          `*Bài hát: **[${player.queue.current.title}](${player.queue.current.uri})`
        )
    ...
    let Pages = ChunkedSongs.map((Tracks) => {
      let SongsDescription = Tracks.map(
        (t) =>
          ...
          )}\` **|** **Tác giả: **\`${t.author}\` **|** **Yêu cầu bởi: **${t.requester}\n`
      ).join("\n");
