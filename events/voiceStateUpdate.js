    ...
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Tiếp tục hàng chờ bị tạm dừng`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Tiếp tục phát lại vì tất cả các bạn đã để lại cho tôi âm nhạc để chơi một mình`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          ...
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`Trình phát nhạc đã bị tạm dừng vì mọi người đã rời đi`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }
